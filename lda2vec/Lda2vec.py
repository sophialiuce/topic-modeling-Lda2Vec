...
class Lda2vec:
    ...
    def __init__(self, num_unique_documents, vocab_size, num_topics, freqs=None, 
        ...
        if not restore:
            # we will run into this by default.
            ...
            if isinstance(pretrained_embeddings, np.ndarray):
                ...
                if fixed_words:
                    ...
                    # we will run into this by default
                    tf.get_variable("word_embedding", shape=[self.vocab_size,self.embedding_size], initializer=tf.constant_initializer(pretrained_embeddings))
                """
